...
/// Default RPC Provider implementation. Conforms to `ArisenRpcProviderProtocol`.
/// RPC Reference: https://developers.rix.io/Arisen-aos/reference
public class ArisenRpcProvider {
    ...
    /// The blockchain ID that all RPC calls for an active instance of ArisenRpcProvider should be interacting with.
    ...
    private func nextActionFor(error: Error) -> NextAction {
        ...
        } else if let theError = error as? ArisenError {
    ...
    /// - Returns: A Promise fulfilling with a response object conforming to the `ArisenRpcResponseProtocol` and rejecting with an Error.
    func getResource<T: Decodable & ArisenRpcResponseProtocol>(_: PMKNamespacer, rpc: String, requestParameters: Encodable?) -> Promise<T> {
    ...
    //  }
    //
    // This particular implementation is typing the return promise to a Decodable & ArisenRpcResponseProtocol object.
    private func runWithFailover<T: Decodable & ArisenRpcResponseProtocol>(_ body: @escaping () -> Promise<T>) -> Promise<T> {
        func failOver() -> Promise<T> {
            return body().recover { error -> Promise<T> in
                ...
                guard self.canErrorFailOverToNewEndpoint(error: error) else {
                    throw ArisenError(.rpcProviderError, reason: error.localizedDescription, originalError: error as NSError)
                }
                return failOver()
            }
        }
    ...
    private func processRequest<T: Decodable & ArisenRpcResponseProtocol>(rpc: String, requestParameters: Encodable?) -> Promise<T> {

        // This promise var is used for the return of Promise<T> expected in this function.
        var promise: Promise<T>
        promise = captureChainId(rpc: rpc).then { (response: ArisenRpcInfoResponse) -> Promise<T> in
    ...
    private func captureChainId(rpc: String) -> Promise<ArisenRpcInfoResponse> {
        var promise: Promise<ArisenRpcInfoResponse>

        if rpc != self.getInfoRpc && self.chainId != nil {
            // Need to return a dummy response object there to satisfy the promise expectation.
            let response = ArisenRpcInfoResponse(chainId: "", headBlockNum: ArisenUInt64.uint64(0),
                                                lastIrreversibleBlockNum: ArisenUInt64.uint64(0),
        ...
        return promise.then { (response: ArisenRpcInfoResponse) -> Promise<ArisenRpcInfoResponse>  in
            if self.chainId == nil && self.originalChainId == nil {
                ...
                if self.originalChainId == response.chainId {
                    ...
                    let error = ArisenError(.rpcProviderChainIdError, reason: "New endpoint chain ID does not match previous endpoint chain ID.")
    ...
    private func runRequestWithRetry<T: Decodable & ArisenRpcResponseProtocol>(rpc: String, requestParameters: Encodable?) -> Promise<T> {
    ...
    private func runRequest<T: Decodable & ArisenRpcResponseProtocol>(rpc: String, requestParameters: Encodable?) -> Promise<T> {

        guard let endpoint = currentEndpoint else {
            let error = ArisenError(.rpcProviderError, reason: "No current endpoint is set.")
            return Promise(error: error)
        }
    ...
    private func buildRequest(rpc: String, endpoint: URL, requestParameters: Encodable?) -> Promise<URLRequest> {
        ...
        if let requestParameters = requestParameters {
            do {
                ...
                let ArisenError = ArisenError(.rpcProviderFatalError, reason: "Error while encoding request parameters.", originalError: error as NSError)
                return Promise(error: ArisenError)
            }
        }
        return Promise.value(request)
    }

    private func decodeResponse<T: Decodable & ArisenRpcResponseProtocol>(data: Data) -> Promise<T> {
        ...
        do {
            ...
            return makeFatalArisenErrorPromiseError(reason: errorReason)
            ...
            return makeFatalArisenErrorPromiseError(reason: errorReasonPrefix, originialError: error as NSError)
        }
    }

    private func makeFatalArisenErrorPromiseError<T>(reason: String, originialError: NSError? = nil) -> Promise<T> {
        let error = ArisenError(.rpcProviderFatalError, reason: reason, originalError: originialError)
        return Promise(error: error)
    ...
    ///   - requestParameters: The request object.
    ///   - callback: Callback.
    func getResource<T: Decodable & ArisenRpcResponseProtocol>(rpc: String, requestParameters: Encodable?, callback: @escaping (T?, ArisenError?) -> Void) {
        getResource(.promise, rpc: rpc, requestParameters: requestParameters)
            .done {
                callback($0, nil)
            }.catch { error in
                var ArisenError: ArisenError
                if let error = error as? ArisenError {
                    ArisenError = error
                } else {
                    ArisenError = ArisenError(.rpcProviderError, reason: "Other error.", originalError: error as NSError)
                }
                callback(nil, ArisenError)
        }
    }
}
