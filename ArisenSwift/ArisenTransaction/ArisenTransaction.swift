...
/// Class for creating, preparing, signing, and (optionally) broadcasting transactions on Arisen-based blockchains.
public class ArisenTransaction: Codable {
    ...
    public var rpcProvider: ArisenRpcProviderProtocol?
    ...
    public var abiProvider: ArisenAbiProviderProtocol?
    ...
    public var signatureProvider: ArisenSignatureProviderProtocol?
    ...
    public var config = ArisenTransaction.Config()
    ...
    /// For encoding/decoding ArisenTransaction <> JSON.
    ...
    /// Deserialize a serialized transaction and return an `ArisenTransaction` object.
    ///
    /// - Parameters:
    ///   - serializedTransaction: A serialized transaction as Data.
    ///   - serializationProvider: A serialization provider. Will be used for transaction deserialization and set as the `serializationProvider` on the returned `ArisenTransaction`.
    /// - Returns: An `ArisenTransaction`.
    /// - Throws: If the transaction cannot be deserialized.
    static public func deserialize(_ serializedTransaction: Data, serializationProvider: ArisenSerializationProviderProtocol) throws -> ArisenTransaction {
        let json = try serializationProvider.deserializeTransaction(hex: serializedTransaction.hex)
        guard let data = json.data(using: .utf8) else {
            throw ArisenError(.deserializeError, reason: "Cannot create json from data")
        }
        let jsonDecoder = JSONDecoder()
        jsonDecoder.dateDecodingStrategy = .formatted(Date.asTransactionTimestamp)
        let transaction = try jsonDecoder.decode(ArisenTransaction.self, from: data)
    ...
    public var actionAccountsMissingAbis: [ArisenName] {
        let accounts = allActions.compactMap { (action) -> ArisenName in
    ...
    public func serializeTransaction() throws -> Data {
        ...
        guard refBlockNum > 0 else {
            throw ArisenError(.serializeError, reason: "refBlockNum is not set")
        }
        guard refBlockPrefix > 0 else {
            throw ArisenError(.serializeError, reason: "refBlockPrefix is not set")
        }
        guard expiration > Date(timeIntervalSince1970: 0) else {
            throw ArisenError(.serializeError, reason: "expiration is not set")
        }
    ...
    ///
    /// - Parameter completion: Called with an `ArisenResult` consisting of `Data` for success and an optional `ArisenError`.
    public func serializeTransaction(completion: @escaping (ArisenResult<Data, ArisenError>) -> Void) {
        prepare { [weak self] (result) in
            guard let strongSelf = self else {
                return completion(.failure(ArisenError(.unexpectedError, reason: "self does not exist")))
            }
            switch result {
            ...
            case .success:
                do {
                    ...
                    return completion(.failure(error.ArisenError))
                }
            }
        }
    }
    ...
    ///
    /// - Parameter completion: Called with an `ArisenResult` consisting of a `Bool` for success and an optional `ArisenError`.
    public func prepare(completion: @escaping (ArisenResult<Bool, ArisenError>) -> Void) {
    ...
    public func serializeActionData() throws {
        ...
        guard missingAbis.count == 0 else {
            throw ArisenError(.serializeError, reason: "Cannot serialize action data. Abis missing for \(missingAbis).")
        }
        guard let serializer = self.serializationProvider else {
            preconditionFailure("A serializationProvider must be set!")
        }
    ...
    ///
    /// - Parameter completion: Called with an `ArisenResult` consisting of a `Bool` for success and an optional `ArisenError`.
    public func serializeActionData(completion: @escaping (ArisenResult<Bool, ArisenError>) -> Void) {
    ...
    public func deserializeActionData(exclude: [ArisenName] = []) throws {
        guard let serializer = self.serializationProvider else {
            preconditionFailure("A serializationProvider must be set!")
        }
        for action in allActions {
    ...
    /// immediately call the completion with `true`. If the `abiProvider` is not set but the `rpcProvider` is, an `ArisenAbiProvider` instance will be created using the `rpcProvider` and set as the `abiProvider`.
    ...
    ///
    /// - Parameter completion: Called with an `ArisenResult` consisting of a `Bool` for success and an optional `ArisenError`.
    public func getAbis(completion: @escaping (ArisenResult<Bool, ArisenError>) -> Void) {
        let missingAbis = actionAccountsMissingAbis
        ...
        if let rpcProvider = self.rpcProvider, self.abiProvider == nil {
            self.abiProvider = ArisenAbiProvider(rpcProvider: rpcProvider)
        }
        guard let abiProvider = self.abiProvider else {
            return completion(.failure(ArisenError(.ArisenTransactionError, reason: "No abi provider available but missing abis for \(missingAbis)")))
        }
        guard chainId != "" else {
            return completion(.failure(ArisenError(.ArisenTransactionError, reason: "Chain id is not set")))
        }
    ...
    ///
    /// - Parameter completion: Called with an `ArisenResult` consisting of a `Bool` for success and an optional `ArisenError`.
    private func getInfoAndSetValues(completion: @escaping (ArisenResult<Bool, ArisenError>) -> Void) {
        ...
        guard let rpcProvider = rpcProvider else {
            return completion(.failure(ArisenError(.ArisenTransactionError, reason: "No rpc provider available")))
        }

        // get chain info
        rpcProvider.getInfo { [weak self] (infoResponse) in
            guard let strongSelf = self else {
                return completion(.failure(ArisenError(.getInfoError, reason: "self does not exist")))
            }
            switch infoResponse {
            case .failure(let error):
                completion(.failure(error))
            case .success(let info):
                ...
                guard strongSelf.chainId == info.chainId else {
                    return completion(.failure(ArisenError(.ArisenTransactionError, reason: "Provided chain id \(strongSelf.chainId) does not match chain id \(info.chainId)")))
                }
                ...
                if strongSelf.expiration <= Date(timeIntervalSince1970: 0) {
                    guard let headBlockTime = Date(yyyyMMddTHHmmss: info.headBlockTime) else {
                        return completion(.failure(ArisenError(.ArisenTransactionError, reason: "Invalid head block time \(info.headBlockTime)")))
                    }
    ...
    ///   - completion: Called with an `ArisenResult` consisting of a `Bool` for success and an optional `ArisenError`.
    public func getBlockAndSetTapos(blockNum: UInt64, completion: @escaping (ArisenResult<Bool, ArisenError>) -> Void) {
        ...
        let requestParameters = ArisenRpcBlockRequest(blockNumOrId: blockNum)

        rpcProvider.getBlock(requestParameters: requestParameters, completion: { [weak self] (blockResponse) in
            guard let strongSelf = self else {
                return completion(.failure(ArisenError(.getBlockError, reason: "self does not exist")))
            }
    ...
    ///   - completion: Called with an `ArisenResult` consisting of a `Bool` for success and an optional `ArisenError`.
    public func sign(prompt: String = "Sign Transaction", completion: @escaping (ArisenResult<Bool, ArisenError>) -> Void) {
        guard let signatureProvider = signatureProvider else {
            return completion(.failure(ArisenError(.signatureProviderError, reason: "No signature provider available")))
        }
        signatureProvider.getAvailableKeys { [weak self] (response) in
            guard let availableKeys = response.keys else {
                return completion(.failure(response.error ?? ArisenError(.signatureProviderError, reason: "Unable to get available keys from signature provider")))
            }
            guard let strongSelf = self else {
                return completion(.failure(ArisenError(.unexpectedError, reason: "self does not exist")))
            }
    ...
    ///   - prompt: Prompt to present with biometrics authentication, if required.
    ///   - completion: Called with an `ArisenResult` consisting of a `Bool` for success and an optional `ArisenError`.
    public func sign(availableKeys: [String], prompt: String = "Sign Transaction", completion: @escaping (ArisenResult<Bool, ArisenError>) -> Void) {
        prepare { [weak self] (result) in
            guard let strongSelf = self else {
                return completion(.failure(ArisenError(.unexpectedError, reason: "self does not exist")))
            }
    ...
    ///   - prompt: Prompt to present with biometrics authentication, if required.
    ///   - completion: Called with an `ArisenResult` consisting of a `Bool` for success and an optional `ArisenError`.
    private func signPreparedTransaction(availableKeys: [String], prompt: String, completion: @escaping (ArisenResult<Bool, ArisenError>) -> Void) {
        guard let rpcProvider = rpcProvider else {
            return completion(.failure(ArisenError(.signatureProviderError, reason: "No rpc provider available")))
        }
        let requiredKeysRequest = ArisenRpcRequiredKeysRequest(availableKeys: availableKeys, transaction: self)
    ...
    ///   - completion: Called with an `ArisenResult` consisting of a `Bool` for success and an optional `ArisenError`.
    public func sign(publicKeys: [String], prompt: String = "Sign Transaction", completion: @escaping (ArisenResult<Bool, ArisenError>) -> Void) {
    ...
    ///   - prompt: Prompt to present with biometric authentication if required.
    ///   - completion: Called with an `ArisenResult` consisting of a `Bool` for success and an optional `ArisenError`.
    private func sign(serializedTransaction: Data, publicKeys: [String], prompt: String, completion: @escaping (ArisenResult<Bool, ArisenError>) -> Void) {
        guard let signatureProvider = signatureProvider else {
            return completion(.failure(ArisenError(.signatureProviderError, reason: "No signature provider available")))
        }
        var transactionSignatureRequest = ArisenTransactionSignatureRequest()
        ...
        var binaryAbis = [ArisenTransactionSignatureRequest.BinaryAbi]()
        for (name, hexAbi) in abis.hexAbis() {
            var binaryAbi = ArisenTransactionSignatureRequest.BinaryAbi()
        ...
        signatureProvider.signTransaction(request: transactionSignatureRequest, prompt: prompt) { [weak self] (transactionSignatureResponse) in
            ...
            guard let signedTransaction = transactionSignatureResponse.signedTransaction else {
                return completion(.failure(transactionSignatureResponse.error ?? ArisenError(.signatureProviderError, reason: "Signature provider error")))
            }
    ...
    /// Process a signed transaction. If the transaction has been modified by the signature provider, it deserializes the signed transaction and updates/sets the `ArisenTransaction` properties. If the
    ...
    ///   - completion: Called with an `ArisenResult` consisting of a `Bool` for success and an optional `ArisenError`.
    private func process(signedTransaction: ArisenTransactionSignatureResponse.SignedTransaction, originalSerializedTransaction: Data, completion: @escaping (ArisenResult<Bool, ArisenError>) -> Void) {
        ...
        guard allowSignatureProviderToModifyTransaction else {
            return completion(.failure(ArisenError(.signatureProviderError, reason: "Signature provider is not allowed to modify transaction")))
        }
        ...
        do {
            let modifiedTransaction = try ArisenTransaction.deserialize(signedTransaction.serializedTransaction, serializationProvider: serializer)
    ...
    ///
    /// - Parameter completion: Called with an `ArisenResult` consisting of a `Bool` for success and an optional `ArisenError`.
    public func broadcast(completion: @escaping (ArisenResult<Bool, ArisenError>) -> Void) {
        guard let serializedTransaction = serializedTransaction, let signatures = signatures, signatures.count > 0 else {
            return completion(.failure(ArisenError(.ArisenTransactionError, reason: "Transaction must be signed before broadcast")))
        }
        guard let rpcProvider = rpcProvider else {
            return completion(.failure(ArisenError(.ArisenTransactionError, reason: "No rpc provider available")))
        }
        var pushTransactionRequest = ArisenRpcPushTransactionRequest()
    ...
    ///
    /// - Parameter completion: Called with an `ArisenResult` consisting of a `Bool` for success and an optional `ArisenError`.
    public func signAndBroadcast(completion: @escaping (ArisenResult<Bool, ArisenError>) -> Void) {
