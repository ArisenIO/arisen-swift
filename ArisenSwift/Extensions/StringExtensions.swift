...
public extension String {
    ...
    /// - Throws: A `ArisenError` if the string can not be converted to a dictionary.
    func jsonToDictionary() throws -> [String: Any] {
        ...
        guard let result = try? JSONSerialization.jsonObject(with: data, options: JSONSerialization.ReadingOptions.allowFragments), let dict = result as? [String: Any] else {
            throw ArisenError(.deserializeError, reason: "Cannot create dictionary from json")
        }
        return dict
    }
    ...
    /// - Throws: ArisenError if the passed object is not convertible to JSON string.
    init<T: Encodable>(encodeToJson: T) throws {
        ...
        if let string = String(data: data, encoding: .utf8) {
            ...
            throw ArisenError(.serializeError, reason: "Cannot create json from data \(data.hexEncodedString())")
        }
    }
