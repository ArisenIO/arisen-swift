...
class ArisenRpcProviderTests: XCTestCase {
    ...
    func test_rpcProvider_shouldNotRetryFor418HttpStatusError() {
        ...
        let requestParameters = ArisenRpcBlockRequest(blockNumOrId: "25260032")
    ...
    func test_rpcProvider_shouldNotRetryFor401HttpStatusError() {
        rpcProvider = ArisenRpcProvider(endpoints: [url, url2, url3], retries: 3)
        var numberOfTimesTried: UInt = 0
        var call = 1
        (stub(condition: isHost("localhost")) { request in
            if call == 1 && request.url?.relativePath == "/v1/chain/get_info" {
    ...
    func test_rpcProvider_shouldRetryBeforeReturningHttpStatusError() {
        rpcProvider = ArisenRpcProvider(endpoint: url, retries: 3)
        var numberOfTimesTried: UInt = 0
        var call = 1
        (stub(condition: isHost("localhost")) { request in
            if call == 1 && request.url?.relativePath == "/v1/chain/get_info" {
    ...
    func testGetInfo() {
        ...
        rpcProvider.getInfo { response in
            switch response {
            case .success(let infoResponse):
                guard let rpcInfoResponse = infoResponse as? ArisenRpcInfoResponse else {
                ...
                let concreteRpcProvider = self.rpcProvider as? ArisenRpcProvider
    ...
    func testGetBlock() {
        ...
        rpcProvider.getBlock(requestParameters: requestParameters) { response in
            switch response {
            case .success(let blockResponse):
                guard let rpcBlockResponse = blockResponse as? ArisenRpcBlockResponse else {
    ...
    func testGetBlockById() {
        ...
        let expect = expectation(description: "testGetBlock")
        let requestParameters = ArisenRpcBlockRequest(blockNumOrId: "0181700002e623f2bf291b86a10a5cec4caab4954d4231f31f050f4f86f26116")
        rpcProvider.getBlock(requestParameters: requestParameters) { response in
            switch response {
            case .success(let blockResponse):
                guard let rpcBlockResponse = blockResponse as? ArisenRpcBlockResponse else {
    ...
    func testGetRawAbiArisen() {
        var callCount = 1
        (stub(condition: isHost("localhost")) { request in
            let name = try? ArisenName("arisen")
        ...
        do {
            ...
            let name = try ArisenName("arisen")
            let requestParameters = ArisenRpcRawAbiRequest(accountName: name)
            rpcProvider.getRawAbi(requestParameters: requestParameters) { response in
                switch response {
                case .success(let rawAbiResponse):
                    guard let rpcRawAbiResponse = rawAbiResponse as? ArisenRpcRawAbiResponse else {
                        return XCTFail("Failed to parse raw abi response")
                    }
                    XCTAssertTrue(rpcRawAbiResponse.accountName == "arisen")
    ...
    func testGetRawAbiToken() {
        var callCount = 1
        (stub(condition: isHost("localhost")) { request in
            let token = try? ArisenName("arisen.token")
        ...
        do {
            let expect = expectation(description: "testGetRawAbi")
            let name = try ArisenName("arisen.token")
            let requestParameters = ArisenRpcRawAbiRequest(accountName: name)
            rpcProvider.getRawAbi(requestParameters: requestParameters) { response in
                switch response {
                case .success(let rawAbiResponse):
                    ...
                    XCTAssertTrue(rpcRawAbiResponse.accountName == "arisen.token")
    ...
    func testGetRequiredKeys() {
        ...
        let requestParameters = ArisenRpcRequiredKeysRequest(availableKeys: ["PUB_K1_5j67P1W2RyBXAL8sNzYcDLox3yLpxyrxgkYy1xsXzVCw1oi9eG"], transaction: transaction)

        rpcProvider.getRequiredKeys(requestParameters: requestParameters) { response in
            switch response {
            case .success(let requiredKeysResponse):
                guard let rpcRequiredKeysResponse = requiredKeysResponse as? ArisenRpcRequiredKeysResponse else {
                    return XCTFail("Failed to parse rpc required key response")
                }
                XCTAssertTrue(rpcRequiredKeysResponse.requiredKeys.count == 1)
                XCTAssertTrue(rpcRequiredKeysResponse.requiredKeys[0] == "RSN5j67P1W2RyBXAL8sNzYcDLox3yLpxyrxgkYy1xsXzVCvzbYpba")
    ...
    func testPushTransaction() {
        ...
        rpcProvider.pushTransaction(requestParameters: requestParameters) { response in
            ...
            case .success(let pushedTransactionResponse):
                ...
                if let resp = pushedTransactionResponse as? ArisenRpcTransactionResponse,
