...
public class RpcTestConstants {
    ...
    */
    ...
    public static func getHHTTPStubsResponse(callCount: Int, urlRelativePath: String?, name: arisenName?, unhappy: Bool = false) -> OHHTTPStubsResponse {
        ...
        if callCount == 1 && urlRelativePath == "/v1/chain/get_info" {
            ...
            case "/v1/chain/get_raw_abi" :
                guard let arisenName = name else {
                    return RpcTestConstants.getErrorOHHTTPStubsResponse(reason: "arisenName missing for call to get_raw_abi stub")
                }
                guard let json = RpcTestConstants.createRawApiResponseJson(account: arisenName) else {
                    return RpcTestConstants.getErrorOHHTTPStubsResponse(code: NSURLErrorUnknown, reason: "Failed to create json in createRawApiResponseJson(account: arisenName)")
                }
    ...
    "core_liquid_balance": "986418.1921 RIX",
    "ram_quota": 13639863,
    "net_weight": 10000000,
    "cpu_weight": 10000000,
    "net_limit": {
    ...
    "permissions": [
        {
            ...
            "required_auth": {
                ...
                "keys": [
                    {
                        "key": "RSN5j67P1W2RyBXAL8sNzYcDLox3yLpxyrxgkYy1xsXzVCvzbYpba",
                        "weight": 1
                    }
                ],
                "accounts": [
        ...
        },
        {
            ...
            "required_auth": {
                ...
                "keys": [
                    {
                        "key": "RSN74RoiasFcUQp1a8qdo3WUoHLGG3NTTts4UcyiqS4qh9eZy1Vxh",
                        "weight": 1
                    }
                ],
                "accounts": [],
    ...
    "total_resources": {
        "owner": "cryptkeeper",
        "net_weight": "1000.0000 RIX",
        "cpu_weight": "1000.0000 RIX",
        "ram_bytes": 13639863
    },
    ...
    public static let currencyStatsRIX = """
    {
    "RIX": {
        "supply": "5000000000.0000 RIX",
        "max_supply": "10000000000.0000 RIX",
    ...
    //Input defaults do not produce a working request, not sure if all fields still valid.  Some requests from [rixjs/4.-Reading blockchain-Examples.md](https://github.com/arisen/rixjs/blob/41bb99b319c1a3b24110a52c3a6e1558df0f326a/docs/4.-Reading%20blockchain-Examples.md) tried.  Some work, some do not.  One working example:
    public static let tableRowsJson = """
    {
    "rows": [
        {
            "balance": "986420.1921 RIX"
        }
    ],
    "more": false
    }
    ...
    "rows": [
        {
            ...
            "producer_key": "RIX8QEPQ1wiZrdHavectXNpMy3TpcyYGXSVfkVGhyVTaJ7FT8bzCZ",
            "is_active": 1,
            "url": "",
            "unpaid_blocks": 0,
        ...
        },
        {
            "owner": "blkproducer3",
            "total_votes": "0.00000000000000000",
            "producer_key": "RIX7YTjjzt7coagRE5i64JhZB4ucH23Nz5mDhXdzU6QC7gfXwy8M9",
            "is_active": 1,
            "url": "",
            "unpaid_blocks": 0,
    ...
    public static func createRawApiResponseJson(account: ArisenName) -> String? {
        let json: String
