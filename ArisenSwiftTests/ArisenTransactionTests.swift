...
class ArisenTransactionTests: XCTestCase {
    ...
    // MARK: - ArisenTransaction extension tests using Promises
    ...
    func test_add_action_shouldSucceed() {
        ...
        guard let action = try? makeTransferAction(from: ArisenName("todd"), to: ArisenName("brandon")) else { return XCTFail("Invalid Action") }
    ...
    func test_add_actions_shouldSucceed() {
        let transaction = ArisenTransaction()
        guard let action1 = try? makeTransferAction(from: ArisenName("todd"), to: ArisenName("brandon")) else { return XCTFail("Invalid Action") }
        guard let action2 = try? makeTransferAction(from: ArisenName("brandon"), to: ArisenName("todd")) else { return XCTFail("Invalid Action") }
...
class RPCProviderMock: ArisenRpcProviderProtocol {
    ...
    let rpcInfo = ArisenRpcInfoResponse(
        serverVersion: "verion",
        chainId: "chainId",
        headBlockNum: ArisenUInt64.uint64(234),
        lastIrreversibleBlockNum: ArisenUInt64.uint64(2342),
        lastIrreversibleBlockId: "lastIrversible",
        headBlockId: "headBlockId",
        headBlockTime: "2009-01-03T18:15:05.000",
        headBlockProducer: "producer",
        virtualBlockCpuLimit: ArisenUInt64.uint64(234),
        virtualBlockNetLimit: ArisenUInt64.uint64(234),
        blockCpuLimit: ArisenUInt64.uint64(334),
        blockNetLimit: ArisenUInt64.uint64(897),
    ...
    func getInfo(completion: @escaping (ArisenResult<ArisenRpcInfoResponseProtocol, ArisenError>) -> Void) {
        ...
        if getInfoReturnsfailure {
            let error = ArisenError(.getInfoError, reason: "Failed for test propose")
    ...
    let block = ArisenRpcBlockResponse(
        ...
        id: "klj",
        blockNum: ArisenUInt64.uint64(89),
        refBlockPrefix: ArisenUInt64.uint64(0980)
    )
    ...
    func getBlock(requestParameters: ArisenRpcBlockRequest, completion: @escaping (ArisenResult<ArisenRpcBlockResponseProtocol, ArisenError>) -> Void) {
        ...
        let result: ArisenResult<ArisenRpcBlockResponseProtocol, ArisenError>
        if getBlockReturnsFailure {
            result = ArisenResult.failure(ArisenError(.getBlockError, reason: "Some reason"))
        } else {
            result = ArisenResult.success(block)
        }
    ...
    public func getRawAbi(requestParameters: ArisenRpcRawAbiRequest, completion: @escaping
        (ArisenResult<ArisenRpcRawAbiResponseProtocol, ArisenError>) -> Void) {
        var result: ArisenResult<ArisenRpcRawAbiResponseProtocol, ArisenError> =
            ArisenResult.failure(ArisenError(.rpcProviderError, reason: "Abi response conversion error."))
        if getRawAbiReturnsfailure {
            result = ArisenResult.failure(ArisenError(.rpcProviderError, reason: "No abis found."))
        } else {
            let decoder = JSONDecoder()
            if let rawAbiResponse = RpcTestConstants.createRawApiResponseJson(account: requestParameters.accountName),
                let resp = try? decoder.decode(ArisenRpcRawAbiResponse.self, from: rawAbiResponse.data(using: .utf8)!) {
                    result = ArisenResult.success(resp)
            }
        }
        completion(result)
    }
    ...
    public func getRequiredKeys(requestParameters: ArisenRpcRequiredKeysRequest, completion: @escaping (ArisenResult<ArisenRpcRequiredKeysResponseProtocol, ArisenError>) -> Void) {
        let result: ArisenResult<ArisenRpcRequiredKeysResponseProtocol, ArisenError>
        if getRequiredKeysReturnsfailure {
            result = ArisenResult.failure(ArisenError(.rpcProviderError, reason: "No required keys found."))
        } else {
            let requredKeysResponse = ArisenRpcRequiredKeysResponse(requiredKeys: ["PUB_K1_5AzPqKAx4caCrRSAuyojY6rRKA3KJf4A1MY3paNVqV5eGGP63Y"])
            result = ArisenResult.success(requredKeysResponse)
        }
        completion(result)
    }
    ...
    public func pushTransaction(requestParameters: ArisenRpcPushTransactionRequest, completion: @escaping (ArisenResult<ArisenRpcTransactionResponseProtocol, ArisenError>) -> Void) {
        let pushTransactionResponse = ArisenRpcTransactionResponse(transactionId: "mocktransactionid")
        let result: ArisenResult<ArisenRpcTransactionResponseProtocol, ArisenError>  = ArisenResult.success(pushTransactionResponse)
        completion(result)
    }
}

final class SerializationProviderMock: ArisenSerializationProviderProtocol {
    var error: String?
    ...
    func serializeTransaction(json: String) throws -> String {
        if serializeTransactionReturnsfailure {
            throw ArisenError(ArisenErrorCode.serializeError, reason: "Test should have expected this failure.")
        } else {
           return ""
        }
    }
...
class AbiProviderMockup: ArisenAbiProviderProtocol {
...
class SignatureProviderMock: ArisenSignatureProviderProtocol {
    ...
    func signTransaction(request: ArisenTransactionSignatureRequest, completion: @escaping (ArisenTransactionSignatureResponse) -> Void) {
        var transactionSignatureResponse = ArisenTransactionSignatureResponse()
        ...
        if request.publicKeys.contains("PUB_K1_5AzPqKAx4caCrRSAuyojY6rRKA3KJf4A1MY3paNVqV5eGGP63Y") {
            ...
            transactionSignatureResponse.error = ArisenError(ArisenErrorCode.signatureProviderError, reason: "Key not available")
        }
    ...
    func getAvailableKeys(completion: @escaping (ArisenAvailableKeysResponse) -> Void) {
        ...
        if getAvailableKeysShouldReturnFailure == true {
            availableKeysResponse.error = ArisenError(ArisenErrorCode.signatureProviderError, reason: "Expected error for testing.")
